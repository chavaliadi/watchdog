...
export const MonitorRow: React.FC<MonitorRowProps> = ({
  ...
  return (
    <tr className="border-b border-zinc-800/60 hover:bg-zinc-800/25 transition-colors group">
      {/* Name & Target */}
      <td className="py-3 px-4">
        <div className="flex flex-col">
          <Link
            to={`/monitors/${monitor.id}`}
            className="font-medium text-sm text-zinc-100 hover:text-blue-400 transition-colors flex items-center gap-1.5"
          >
            <span>{monitor.name}</span>
            <ArrowUpRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 text-zinc-500 transition-opacity" />
          </Link>
      ...
      <td className="py-3 px-4 whitespace-nowrap font-mono text-xs text-zinc-300">
        <div className="flex items-center gap-1">
          <span className="text-zinc-200 font-semibold">{formatDuration(monitor.interval_ms)}</span>
      ...
      <td className="py-3 px-4 whitespace-nowrap">
        <label className="relative inline-flex items-center cursor-pointer">
          <input
            ...
            aria-label={`Toggle active state for ${monitor.name}`}
          />
      ...
      <td className="py-3 px-4 whitespace-nowrap text-right text-xs">
        <div className="flex items-center justify-end gap-1">
          <button
            ...
            className="p-1.5 rounded-md text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/80 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 cursor-pointer"
          ...
          <button
            ...
            className="p-1.5 rounded-md text-zinc-400 hover:text-rose-400 hover:bg-zinc-800/80 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 cursor-pointer"
